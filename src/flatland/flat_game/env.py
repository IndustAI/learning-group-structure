...
class Env(object):
    ...
    def handle_collisions(self):
        ...
        def begin_wall_collision(arbiter, space, *args, **kwargs):
            ...
            if self.agent.body.position[0]<=10 + self.agent.radius:
                self.agent.body.position = (83-self.agent.body.position[0],self.agent.body.position[1])
                #self.agent.body.position = (57.9,self.agent.body.position[1])

            if self.agent.body.position[0]>=74 - self.agent.radius:
                self.agent.body.position = (self.agent.body.position[0]-(63-2*self.agent.radius),self.agent.body.position[1])
                #self.agent.body.position = (26.1,self.agent.body.position[1])

            if self.agent.body.position[1]<=10 + self.agent.radius:
                self.agent.body.position = (self.agent.body.position[0],83-self.agent.body.position[1])
                #self.agent.body.position = (self.agent.body.position[0],57.9)

            if self.agent.body.position[1]>=74 - self.agent.radius:
                self.agent.body.position = (self.agent.body.position[0],self.agent.body.position[1]-(63-2*self.agent.radius))
                #self.agent.body.position = (self.agent.body.position[0],26.1)
    ...
    def step(self, action):
        ...
        return self.npimage[:, :, 0], self.agent.get_reward(), self.done, self.agent.get_meas()

    def reset(self, position=(30, 30)):
        self.parameters['agent'].update(self.agent.get_new_averages())
        self.parameters['agent'].update({"position": position})
        for sensor in self.agent.sensors:
            sensor.reset()
        self.__init__(**self.parameters)
        self.reload_screen()
        return self.npimage[:, :, 0]
